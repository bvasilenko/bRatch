...
// In-place overwrites of identical size are not detected; accepted because apply()
// is never called on a failed check, making zero-byte-delta writes impossible.
fn install_dir_entries() -> BTreeSet<(std::ffi::OsString, u64)> {
    let dir = cargo_bin("bratch")
        .parent()
...
fn update_subcommand_uses_placeholder_url_when_env_var_absent() {
    // .invalid is NXDOMAIN by RFC 6761 -- no test server needed.
    bratch_command()
...
fn update_subcommand_fails_when_manifest_signature_verification_fails() {
    let manifest_json = serde_json::json!({
        ...
        "platforms": {
            "linux-x86_64": { "archive_url": "https://example.com/bratch.tar", "sha256": "abc" }
        },
        "corpus_version": 1,
...
// A failed check returns before apply() is called -- no file writes are possible.
#[test]
...
// A connection failure (NXDOMAIN) also short-circuits before apply() -- no file writes possible.
// RFC 6761 reserves `.invalid` as a guaranteed NXDOMAIN TLD; no real server is needed.
#[test]
...
// Malformed body causes check() to fail before apply() -- no file writes are possible.
#[test]
