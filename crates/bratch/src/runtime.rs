...
pub struct BinaryRuntime {
    pub corpus: RegressionSignatureCorpusIndex,
...
fn load_corpus() -> Result<RegressionSignatureCorpusIndex, BsuiteCoreError> {
...
) -> Result<RegressionSignatureCorpusIndex, BsuiteCoreError> {
    RegressionSignatureCorpusIndex::from_toml_signed(CORPUS_TOML, pubkey)
...
fn load_defaults(install_dir: &Path) -> Result<BinaryDefaults, BsuiteCoreError> {
    let base_dir = FileSystemTranscriptAppender::new("bratch")?
        .directory()
        .to_path_buf();
    let overlay_reader = FileSystemManifestOverlayReader::new("bratch", install_dir);
